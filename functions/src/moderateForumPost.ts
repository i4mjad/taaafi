...
const LOCALIZED_MESSAGES: LocalizedMessages = {
  arabic: {
    ...
    manual_review: 'منشورك تحت المراجعة من قبل الإدارة'
  },
  english: {
    ...
    manual_review: 'Your post is under review by moderators'
  }
};
...
/**
 * Moderation prompts for Forum Posts
 */
const MODERATION_PROMPTS = {
  arabic: `أنت نظام مراقبة محتوى متخصص في منشورات المنتدى (العنوان + النص) في تطبيق التعافي من الإدمان.

**السياق المهم:**
هذا منشور عام في منتدى الدعم. يجب أن يكون المحتوى داعماً وآمناً للجميع.
...
**النص المطلوب تحليله (العنوان + النص):**
"{{MESSAGE_TEXT}}"
...
مهم: كن متوازناً. احجب المحتوى الخطير أو محاولات التواصل عبر المنصات الخارجية فقط، ولكن اسمح بدعوات المجموعات الداخلية والتعبير الصادق عن المشاعر.`,
  ...
  english: `You are a content moderation system for FORUM POSTS (title + body) in a recovery app.

**Important Context:**
This is a public post in a support community. Content should be supportive and safe for everyone.
...
function getLocalizedMessage(violationType: string, locale: 'arabic' | 'english'): string {
  const messages = LOCALIZED_MESSAGES[locale as 'arabic' | 'english'] || LOCALIZED_MESSAGES.arabic;
...
function evaluateCustomRules(normalizedText: string): CustomRuleResult[] {
  console.log('🔍 Evaluating custom rules for Forum Post context...');
  ...
  if (!isAdministrativeContext && hasIntent && hasContactToken) {
    ...
    results.push({
      ...
      severity: 'medium',
      confidence: 0.8,
      reason: `Detected potential social media promotion in forum post: ${combinedSpans.map(s => s.content).join(', ')}`,
...
async function checkWithOpenAI(text: string): Promise<OpenAIModerationResult> {
  console.log('🤖 Starting OpenAI analysis for Forum Post...');
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a content moderation expert for forum posts. Always respond with valid JSON only.'
        },
        {
          role: 'user',
          content: prompt
...
function synthesizeDecision(
  ...
  console.log('⚖️ Synthesizing final moderation decision for Forum Post...');
...
/**
 * Main Cloud Function for Forum Post Moderation
 */
export const moderateForumPost = onDocumentCreated(
  ...
  async (event) => {
    ...
    console.log('🚀 FORUM POST MODERATION STARTED for:', postId);
    ...
    try {
      ...
      const userLocale = await getUserLocale(post.authorCpId);
      ...
      try {
        ...
        // Hide posts automatically when reviewers flagged with high confidence (>= 0.85)
      ...
      console.log(`\n🏁 FORUM POST MODERATION COMPLETED in ${totalProcessingTime}ms`);

    } catch (error) {
      console.error('💥 CRITICAL ERROR in Forum Post moderation:', error);
      ...
      try {
        ...
        try {
          const post = snap.data() as PostData;
          if (post?.authorCpId) {
            errorLocale = await getUserLocale(post.authorCpId);
          }
        } catch (localeError) {
