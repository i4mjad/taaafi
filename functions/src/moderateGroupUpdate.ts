...
const LOCALIZED_MESSAGES: LocalizedMessages = {
  arabic: {
    ...
    manual_review: 'تحديثك تحت المراجعة من قبل الإدارة'
  },
  english: {
    ...
    manual_review: 'Your update is under review by moderators'
  }
};
...
/**
 * Moderation prompts for Group Updates
 */
const MODERATION_PROMPTS = {
  arabic: `أنت نظام مراقبة محتوى متخصص في تحديثات المجموعات في تطبيق التعافي من الإدمان.
...
function evaluateCustomRules(normalizedText: string): CustomRuleResult[] {
  console.log('🔍 Evaluating custom rules for Group Update context...');
  ...
  if (!isAdministrativeContext && hasIntent && hasContactToken) {
    ...
    results.push({
      ...
      severity: 'medium',
      confidence: 0.8,
      reason: `Detected potential social media promotion in update: ${combinedSpans.map(s => s.content).join(', ')}`,
...
async function checkWithOpenAI(text: string): Promise<OpenAIModerationResult> {
  console.log('🤖 Starting OpenAI analysis for Group Update...');
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a content moderation expert for group updates. Always respond with valid JSON only.'
        },
        {
          role: 'user',
          content: prompt
...
function synthesizeDecision(
  ...
  console.log('⚖️ Synthesizing final moderation decision for Group Update...');
...
/**
 * Main Cloud Function for Group Update Moderation
 */
export const moderateGroupUpdate = onDocumentCreated(
  ...
  async (event) => {
    ...
    try {
      ...
      try {
        ...
        // If manual review with high confidence (>= 0.85), keep visible but redacted
        // If manual review with low confidence (< 0.85), hide completely
        if (finalStatus === 'manual_review') {
            const confidence = finalDecision.confidence || 0;
            updateData.isHidden = confidence < 0.85; // Only hide if low confidence
        }
      ...
      console.log(`\n🏁 GROUP UPDATE MODERATION COMPLETED in ${totalProcessingTime}ms`);

    } catch (error) {
      console.error('💥 CRITICAL ERROR in Group Update moderation:', error);
      
      try {
        let errorLocale: 'arabic' | 'english' = 'arabic';
        try {
          const update = snap.data() as UpdateData;
          if (update?.authorCpId) {
            errorLocale = await getUserLocale(update.authorCpId);
          }
        } catch (localeError) {
