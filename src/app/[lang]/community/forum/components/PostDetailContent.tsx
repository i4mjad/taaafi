...
import { UserReport, REPORT_TYPE_IDS } from '@/types/reports';
...
export default function PostDetailContent({ postId, lang, hideHeader }: PostDetailContentProps) {
  ...
  const [editFormData, setEditFormData] = useState({ title: '', body: '', category: '' });
  ...
  // Fetch reports for post and comments
  ...
  // Fetch profiles for author details
  ...
  const interactions = useMemo(() => {
    if (!interactionsValue) return [] as Interaction[];
    return interactionsValue.docs.map(docSnap => ({
      id: docSnap.id,
      ...docSnap.data(),
  ...
  const comments = useMemo(() => {
    ...
    return commentsValue.docs.map(docSnap => ({
      id: docSnap.id,
      ...docSnap.data(),
      createdAt: docSnap.data().createdAt?.toDate() || new Date(),
      updatedAt: docSnap.data().updatedAt?.toDate(),
  ...
  const categories = useMemo(() => {
    if (!categoriesValue) return [] as { id: string; name: string }[];
    return categoriesValue.docs.map(docSnap => ({
      id: docSnap.id,
  ...
  const postReports = useMemo(() => {
    if (!postReportsValue) return [] as UserReport[];
    return postReportsValue.docs.map(docSnap => ({
      id: docSnap.id,
      ...docSnap.data(),
  ...
  const commentReports = useMemo(() => {
    if (!commentReportsValue) return [] as UserReport[];
    return commentReportsValue.docs.map(docSnap => ({
      id: docSnap.id,
      ...docSnap.data(),
      time: docSnap.data().time?.toDate() || new Date(),
      lastUpdated: docSnap.data().lastUpdated?.toDate() || new Date(),
  ...
  const filteredCommentReports = useMemo(() => {
    ...
    return commentReports.filter(r => r.targetId && commentIds.includes(r.targetId));
  ...
  const getCommentPendingReportCount = (commentId: string) => {
    return filteredCommentReports.filter(r => r.targetId === commentId && r.status === 'pending').length;
  };

  // Edit helpers
  const initializeEditForm = () => {
    if (!post) return;
    setEditFormData({ title: post.title, body: post.body, category: post.category });
    setEditErrors({});
  ...
  const validateEditForm = (): boolean => {
    ...
    if (!editFormData.title.trim()) newErrors.title = t('modules.community.posts.errors.titleRequired');
    if (!editFormData.body.trim()) newErrors.body = t('modules.community.posts.errors.bodyRequired');
    if (!editFormData.category) newErrors.category = t('modules.community.posts.errors.categoryRequired');
  ...
  const usePostUpdate = () => {
    ...
    const updatePost = async (pid: string, data: Partial<typeof editFormData>) => {
      setIsUpdating(true);
      try {
        await updateDoc(doc(db, 'forumPosts', pid), { ...data, updatedAt: new Date() });
  ...
  const handleDeletePost = async () => {
    if (!post) return;
    await updateDoc(doc(db, 'forumPosts', post.id), { isDeleted: true, updatedAt: new Date() });
    setShowDeletePostDialog(false);
  };

  const handleHidePost = async () => {
    if (!post) return;
    await updateDoc(doc(db, 'forumPosts', post.id), { isHidden: !post.isHidden, updatedAt: new Date() });
    setShowHidePostDialog(false);
  };

  const handlePinPost = async () => {
    if (!post) return;
    await updateDoc(doc(db, 'forumPosts', post.id), { isPinned: !post.isPinned, updatedAt: new Date() });
    setShowPinPostDialog(false);
  };

  const openModerationForPost = () => {
    if (!post) return;
    setModerationTarget({ kind: 'post', id: post.id, title: post.title, authorCPId: post.authorCPId, isHidden: post.isHidden, isDeleted: post.isDeleted });
    setModerationOpen(true);
  };

  const openModerationForComment = (comment: Comment) => {
    setModerationTarget({ kind: 'comment', id: comment.id, title: undefined, authorCPId: comment.authorCPId, isHidden: comment.isHidden, isDeleted: comment.isDeleted });
    setModerationOpen(true);
  };
  ...
  const handleDeleteComment = async () => {
    ...
    await updateDoc(doc(db, 'comments', selectedComment.id), { isDeleted: true, updatedAt: new Date() });
  ...
  const handleHideComment = async () => {
    if (!selectedComment) return;
    await updateDoc(doc(db, 'comments', selectedComment.id), { isHidden: !selectedComment.isHidden, updatedAt: new Date() });
  ...
  if (postError || !post) {
    return (
      <div className="p-6 text-center">
        <h1 className="text-2xl font-bold text-destructive mb-4">{t('common.error')}</h1>
        <p className="text-muted-foreground mb-4">{t('modules.community.posts.postNotFound')}</p>
        <Button asChild>
          <Link href={`/${lang}/community/forum`}>{t('modules.community.posts.detailPage.backToList')}</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      {!hideHeader && (
        <div className="flex items-center space-x-4">
          <Button variant="ghost" asChild className="px-2">
            <Link href={`/${lang}/community/forum`}>
              {t('modules.community.posts.detailPage.backToList')}
            </Link>
          </Button>
          <div>
            <h1 className="text-2xl font-bold tracking-tight">{t('modules.community.posts.detailPage.title')}</h1>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center space-x-6 text-sm">
                <div className="flex items-center space-x-1">
                  ...
                  <span>
                    {t('modules.community.posts.likes').replaceAll('{count}', post.likeCount.toString())}
                  </span>
                </div>
                <div className="flex items-center space-x-1">
                  <ThumbsDown className="h-4 w-4 text-red-600" />
                  <span>
                    {t('modules.community.posts.dislikes').replaceAll('{count}', post.dislikeCount.toString())}
                  </span>
                </div>
                <div className="flex items-center space-x-1">
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-3 text-xs text-muted-foreground">
                              <div className="flex items-center space-x-1">
                                ...
                                <span>
                                  {t('modules.community.posts.likes').replaceAll('{count}', comment.likeCount.toString())}
                                </span>
                              </div>
                              <div className="flex items-center space-x-1">
                                <ThumbsDown className="h-3 w-3 text-red-600" />
                                <span>
                                  {t('modules.community.posts.dislikes').replaceAll('{count}', comment.dislikeCount.toString())}
                                </span>
                              </div>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => { setSelectedComment(comment); setShowHideCommentDialog(true); }}>
                                <EyeOff className="mr-2 h-4 w-4" />
                                {comment.isHidden ? t('modules.community.posts.detailPage.commentActions.unhide') : t('modules.community.posts.detailPage.commentActions.hide')}
                              </DropdownMenuItem>
                              ...
                              {!comment.isDeleted && (
                                <DropdownMenuItem className="text-destructive" onClick={() => { setSelectedComment(comment); setShowDeleteCommentDialog(true); }}>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              <Button variant="outline" className="w-full justify-start" onClick={initializeEditForm} disabled={isEditingPost}>
                <Edit className="mr-2 h-4 w-4" />
                {t('modules.community.posts.detailPage.actions.editPost')}
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => setShowHidePostDialog(true)}>
                {post.isHidden ? <Eye className="mr-2 h-4 w-4" /> : <EyeOff className="mr-2 h-4 w-4" />}
                {post.isHidden ? t('modules.community.posts.detailPage.actions.unhidePost') : t('modules.community.posts.detailPage.actions.hidePost')}
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={openModerationForPost}>
                <AlertCircle className="mr-2 h-4 w-4" />
                {t('modules.community.posts.moderation.quickModerate')}
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => setShowPinPostDialog(true)}>
                {post.isPinned ? <PinOff className="mr-2 h-4 w-4" /> : <Pin className="mr-2 h-4 w-4" />}
                {post.isPinned ? t('modules.community.posts.detailPage.actions.unpinPost') : t('modules.community.posts.detailPage.actions.pinPost')}
              </Button>
              ...
              {!post.isDeleted && (
                <Button variant="destructive" className="w-full justify-start" onClick={() => setShowDeletePostDialog(true)}>
                  <Trash2 className="mr-2 h-4 w-4" />
          ...
          <Card>
            ...
            <CardContent>
              {(postReports.length === 0 && filteredCommentReports.length === 0) ? (
                <p className="text-sm text-muted-foreground text-center py-4">{t('modules.community.posts.detailPage.noReports')}</p>
              ) : (
                <div className="space-y-4">
                  {postReports.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium mb-2">{t('modules.community.posts.detailPage.postReports')} ({postReports.length})</h4>
                          ...
                          <div key={report.id} className="flex items-center justify-between text-sm border rounded p-2">
                            <div className="flex-1">
                              <div className="flex items-center space-x-2">
                                <Badge variant={report.status === 'pending' ? 'destructive' : report.status === 'inProgress' ? 'default' : report.status === 'closed' ? 'outline' : 'secondary'} className="text-xs">
                                  {report.status}
                                </Badge>
                                <span className="text-xs text-muted-foreground">{format(report.time instanceof Date ? report.time : report.time.toDate(), 'MMM dd')}</span>
                              </div>
                              <p className="text-xs text-muted-foreground mt-1 truncate">{report.initialMessage.substring(0, 60)}...</p>
                            </div>
                  ...
                  {filteredCommentReports.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium mb-2">{t('modules.community.posts.detailPage.commentReports')} ({filteredCommentReports.length})</h4>
                      <div className="space-y-2">
                          ...
                          return (
                            <div key={report.id} className="flex items-center justify-between text-sm border rounded p-2">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-muted-foreground mt-1 truncate">{t('modules.community.posts.detailPage.commentByAuthor').replaceAll('{author}', relatedComment?.authorCPId || 'Unknown')}</p>
                              </div>
                              <Button variant="ghost" size="sm" asChild className="h-6 w-6 p-0">
                                <Link href={`/${lang}/user-management/reports/${report.id}`}>
                                  <ExternalLink className="h-3 w-3" />
          ...
          <Card>
            ...
            <CardContent>
              {interactions.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">{t('modules.community.posts.detailPage.noInteractions')}</p>
              ) : (
                <div className="space-y-2">
                  {interactions.slice(0, 10).map((interaction) => (
                    <div key={interaction.id} className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span className="text-xs text-muted-foreground">{format(interaction.createdAt, 'MMM dd')}</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <Dialog open={showDeletePostDialog} onOpenChange={setShowDeletePostDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('modules.community.posts.detailPage.deleteConfirm.description')}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDeletePostDialog(false)}>{t('common.cancel')}</Button>
            <Button variant="destructive" onClick={handleDeletePost}>{t('common.delete')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showHidePostDialog} onOpenChange={setShowHidePostDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('modules.community.posts.detailPage.hideConfirm.description')}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowHidePostDialog(false)}>{t('common.cancel')}</Button>
            <Button onClick={handleHidePost}>{post?.isHidden ? t('modules.community.posts.detailPage.actions.unhidePost') : t('modules.community.posts.detailPage.actions.hidePost')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showPinPostDialog} onOpenChange={setShowPinPostDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{post?.isPinned ? t('modules.community.posts.detailPage.actions.unpinPost') : t('modules.community.posts.detailPage.actions.pinPost')}</DialogTitle>
            <DialogDescription>{post?.isPinned ? t('modules.community.posts.detailPage.unpinConfirm.description') : t('modules.community.posts.detailPage.pinConfirm.description')}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowPinPostDialog(false)}>{t('common.cancel')}</Button>
            <Button onClick={handlePinPost}>{post?.isPinned ? t('modules.community.posts.detailPage.actions.unpinPost') : t('modules.community.posts.detailPage.actions.pinPost')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showDeleteCommentDialog} onOpenChange={setShowDeleteCommentDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('modules.community.posts.detailPage.deleteCommentConfirm.description')}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowDeleteCommentDialog(false)}>{t('common.cancel')}</Button>
            <Button variant="destructive" onClick={handleDeleteComment}>{t('common.delete')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showHideCommentDialog} onOpenChange={setShowHideCommentDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('modules.community.posts.detailPage.hideCommentConfirm.description')}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowHideCommentDialog(false)}>{t('common.cancel')}</Button>
            <Button onClick={handleHideComment}>{selectedComment?.isHidden ? t('modules.community.posts.detailPage.commentActions.unhide') : t('modules.community.posts.detailPage.commentActions.hide')}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      {moderationTarget && (
        <ModerationActionDialog
          ...
          onOpenChange={(open) => { setModerationOpen(open); if (!open) setModerationTarget(null); }}
