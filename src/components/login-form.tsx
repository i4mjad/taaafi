...
export function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect to dashboard for current locale
  ...
  return (
    <div className={cn("flex flex-col gap-6", className)} {...props}>
      <form onSubmit={handleSubmit}>
        <div className="flex flex-col gap-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Button variant="outline" type="button" className="w-full" onClick={async () => { await signInWithApple(); }} disabled={loading}>
            ...
            <Button variant="outline" type="button" className="w-full" onClick={async () => { await signInWithGoogle(); }} disabled={loading}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                <path
      ...
      <div className="text-muted-foreground *:[a]:hover:text-primary text-center text-xs text-balance *:[a]:underline *:[a]:underline-offset-4">
        ...
        <a href="#">{dict.login.termsOfService}</a>{" "}
        {dict.login.and}{" "}
        <a href="#">{dict.login.privacyPolicy}</a>.
      </div>
    </div>
  )
}
