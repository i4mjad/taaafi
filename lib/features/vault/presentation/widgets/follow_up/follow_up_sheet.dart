  ...
  Future<void> _saveFollowUpsAndEmotions() async {
    ...
    final followUps = selectedFollowUps.expand((option) {
      if (option.translationKey == 'relapse' && addAllFollowUps) {
        // Filter out the relapse option since it's already in selectedFollowUps
        return followUpOptions
            .where((opt) => opt.translationKey != 'relapse')
            .map((opt) {
          return FollowUpModel(
            id: '', // ID will be generated by Firestore
            type: _mapTranslationKeyToType(opt.translationKey),
            time: widget.date,
            triggers: _shouldIncludeTriggers(opt.translationKey)
        ...
        return [
          FollowUpModel(
            id: '', // ID will be generated by Firestore
            type: _mapTranslationKeyToType(option.translationKey),
            time: widget.date,
            triggers: _shouldIncludeTriggers(option.translationKey)
                ? selectedTriggers.toList()
                : [],
          )
        ];
